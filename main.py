...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/keshrihospital'
db=SQLAlchemy(app)
...
def hospitallogin():
    if request.method=="POST":
        ...
        if user and check_password_hash(user.password,password):
            login_user(user)
            flash("Login Success","info")
            return render_template("hospitalindexpage.html")
        else:
            flash("Invalid Credentials","danger")
...
def hospitalUser():
    ...
    if('user' in session and session['user']==params['user']):
        ...
        return render_template("addHosUser.html")
    #Test karne ke liye ki database sai connect hai ki nahi
@app.route("/test")
def test():
    try:
        a=Test.query.all()
